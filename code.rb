...
puts people["Brian Heller"]["children"].size
...
sets_of_people.each do |set|
  if set.include?("Ruby Hessel")
    puts set["Ruby Hessel"]["phone"]
  end
end

puts "\nWhat are the names of Dr. Sigrid Nader's children?"
sets_of_people.each do |set|
  if !set["Dr. Sigrid Nader"].nil?
    puts set["Dr. Sigrid Nader"]["children"]
  end
end

puts "\nWhat is Bernard Feil's child's name?"
sets_of_people.each do |set|
  if set.include?("Bernard Feil")
    puts set["Bernard Feil"]["children"]
  end
end

puts "\nWhat company does Casandra Kemmer work for?"
sets_of_people.each do |set|
  if !set["Casandra Kemmer"].nil?
    puts set["Casandra Kemmer"]["company"]
  end
end

puts "\nWho are all the people in the first set that have an `m` in their Company names?"
sets_of_people[0].each do |name, info|
  if info["company"].include?("m") || info["company"].include?("M")
    puts name
  end
end
